...
public class WxLoginServiceImpl implements WxLoginService{
	 ...
	 private String appSecret="abd4247f4848c338717e0a556c3c419e";
		...
		public String loginUser(String code) {
			...
			if(res != null && res.getStatusCode() == HttpStatus.OK) {
				...
				if(wxLgn.existsById(wxSession.getOpenid())) {
					wxLgn.updateOne(wxSession.getOpenid(), wxSession.getSession_key(), wxSession.getThirdSession(),wxSession.getUnionid());
				}else {
		...
		public String setUInfo( WxUser wxUser) {
			...
			wxLgn.updateBy3rd(wxUser.getAvatarurl(),wxUser.getNickname(),wxUser.getTelnum(),wxUser.getGender(),wxUser.getLocation(),wxUser.getBirthday(),wxUser.getProvince(),wxUser.getCity(),wxUser.getCountry(),wxUser.getThirdSession());
			return "ok";
		}
}